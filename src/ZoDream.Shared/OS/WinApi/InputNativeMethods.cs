...
namespace ZoDream.Shared.OS.WinApi
{
    public static class InputNativeMethods
    {
        ...
        /// </remarks>
        [DllImport("user32.dll", SetLastError = true)]
        public static extern uint SendInput(uint numberOfInputs, [MarshalAs(UnmanagedType.LPArray), In] InputStruct[] inputs, int sizeOfInputStructure);


        [DllImport("user32.dll", SetLastError = true)]
        public static extern uint SendInput(uint numberOfInputs, [MarshalAs(UnmanagedType.LPArray), In] InputStruct32[] inputs, int sizeOfInputStructure);


        public static uint SendInput(InputStruct[] inputs)
        {
            if (IntPtr.Size > 4)
            {
                return SendInput((uint)inputs.Length, inputs, Marshal.SizeOf(typeof(InputStruct)));
            }
            return SendInput((uint)inputs.Length, inputs.Select(i => new InputStruct32()
            {
                Type = i.Type,
                Keyboard = i.Keyboard,
                Hardware = i.Hardware,
                Mouse = i.Mouse,
            }).ToArray(), Marshal.SizeOf(typeof(InputStruct32)));
        }

        public static uint SendInput(InputStruct32[] inputs)
        {
            if (IntPtr.Size <= 4)
            {
                return SendInput((uint)inputs.Length, inputs, Marshal.SizeOf(typeof(InputStruct32)));
            }
            return SendInput((uint)inputs.Length, inputs.Select(i => new InputStruct()
            {
                Type = i.Type,
                Keyboard = i.Keyboard,
                Hardware = i.Hardware,
                Mouse = i.Mouse,
            }).ToArray(), Marshal.SizeOf(typeof(InputStruct)));
        }
