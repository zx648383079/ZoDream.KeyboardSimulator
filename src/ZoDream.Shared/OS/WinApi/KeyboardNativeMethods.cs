...
namespace ZoDream.Shared.OS.WinApi
{
    internal static class KeyboardNativeMethods
    {
        ...
        internal static void TryGetCharFromKeyboardState(int virtualKeyCode, int fuState, out char[] chars)
        {
            ...
            var scanCode = MapVirtualKeyEx(virtualKeyCode, (int)MapKeyType.VK_TO_VSC, dwhkl);
        ...
        public static void KeyDown(Key key)
        {
            keybd_event((byte)key, (byte)InputNativeMethods.MapVirtualKey((uint)key,
                (uint)MapKeyType.VK_TO_VSC), 0x0001 | 0, 0);
        }

        public static void KeyUp(Key key)
        {
            keybd_event((byte)key, (byte)InputNativeMethods.MapVirtualKey((uint)key,
                (uint)MapKeyType.VK_TO_VSC), 0x0001 | 0x0002, 0);
        }
    }
}
